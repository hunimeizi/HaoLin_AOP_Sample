package com.haolin.aop.sample.aspect;

import android.util.Log;

import com.haolin.aop.sample.annotation.ClickBehavior;
...
public class ClickBehaviorAspect {
    ...
    private final static String TAG = "lyb >>> ";

    // 1、应用中用到了哪些注解，放到当前的切入点进行处理（找到需要处理的切入点）
    // execution，以方法执行时作为切点，触发Aspect类
    // * *(..)) 可以处理ClickBehavior这个类所有的方法
    @Pointcut("execution(@com.haolin.aop.sample.annotation.ClickBehavior * *(..))")
